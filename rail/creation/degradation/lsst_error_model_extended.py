...
class LSSTErrorModel_extended(Degrader):
    """
    ...
    def __init__(
        ...
        if errortype is not None:
            self.settings["errortype"] = errortype
    ...
    def _validate_settings(self):
        ...
        if self.settings["errortype"] not in ["point", "gaap", "auto"]:
            raise ValueError("errortype should be onr of 'point', 'gaap', 'auto'")
    ...
    def _get_area_ratio_gaap(self, majors: np.ndarray, minors: np.ndarray, 
        ...
        """
        theta_size = np.array([self.settings["theta"][band] for band in bands])
        
        hl_to_sigma = 1/0.675
        fwhm_to_sigma = 1/2.355
        
        theta_sigma = theta_size * fwhm_to_sigma
        A_min_sigma = self.settings["A_min"] * fwhm_to_sigma
        A_max_sigma = self.settings["A_max"] * fwhm_to_sigma
    ...
    def _get_obs_and_errs(
        ...
        if self.settings["errortype"] == "gaap":
            a_ratio = self._get_area_ratio_gaap(majors, minors, bands) 
        elif self.settings["errortype"] == "auto":
